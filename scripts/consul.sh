...
if [ -z "$join_ip" ]; then
	docker run --dns $bridge_ip --name consul -h $HOSTNAME  -p $node_ip:8300:8300 -p $node_ip:8301:8301 \
...
docker run  --dns $bridge_ip -d --link consul:consul --name registrator -v /var/run/docker.sock:/tmp/docker.sock -h $HOSTNAME gliderlabs/registrator consul://consul:8500
